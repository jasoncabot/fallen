import { TurnMode } from './game';

export type StructureCategory = 'WALL' | 'HTUR' | 'HAIR' | 'HBAR' | 'HBAY' | 'HDEF' | 'HENY' | 'HFAC' | 'HLAB' | 'HMIN' | 'HRAD' | 'HSHP' | 'HSIL' | 'ATUR' | 'AAIR' | 'ABAR' | 'ABAY' | 'ADEF' | 'AENY' | 'AFAC' | 'ALAB' | 'AMIN' | 'ARAD' | 'ASHP' | 'ASIL';
...
export type Alliance = 'HUMAN' | 'ALIEN' | 'NEUTRAL';
type Placement = 'B-WALL' | 'B-WALL-OR-ROAD' | 'B-ROAD' | 'ANYWHERE';
export type StructureTiles = 'structure-build' | 'structure-infra' | 'structure-human' | 'structure-human-dropship' | 'structure-alien' | 'structure-alien-dropship';
...
export type StructureActionKind = 'FIRE' | 'BUILD_DROPSHIP' | 'EXIT_STRUCTURE' | 'LAUNCH' | 'MISSILE';

export interface StructureProduction {
  category: 'TROOP' | 'FACTORY' | 'HOVER' | 'DROPSHIP' | 'ENERGY' | 'RESEARCH' | 'CREDITS' | 'NONE';
  value: number; // max number of units this structure can hold / produce?
}
...
export interface StructureValue {
  ...
  action: Record<TurnMode, StructureActionKind | null>;
