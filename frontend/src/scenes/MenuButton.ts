...
export class MenuButton extends Phaser.GameObjects.Graphics {
    ...
    constructor(scene: Phaser.Scene, rect: any, text: string, callback: (plugin: Phaser.Scenes.ScenePlugin) => void) {
        let x = rect.x;
        let y = rect.y;
        super(scene, { x, y });
        ...
        this.text = scene.add.text(x, y + 8, text, { font: "16px Verdana", color: style.text.rest });
