...
interface CustomButton extends Phaser.GameObjects.Image {
    highlighted: boolean;
...
const createButton = (scene: Phaser.Scene, x: number, y: number, config: any, callback: (button: Phaser.GameObjects.Image) => void) => {
    ...
    button.on('pointerover', (_pointer: Phaser.Input.Pointer, _x: number, _y: number, event: any) => {
        if (!button.data.get('highlighted') && button.state !== 3) {
    ...
    button.on('pointerdown', (_pointer: Phaser.Input.Pointer, _x: number, _y: number, event: any) => {
        button.setFrame(config.name + '_2');
        event.stopPropagation();
    });
    button.on('pointerup', (_pointer: Phaser.Input.Pointer, _x: number, _y: number, event: any) => {
        button.setFrame(config.name + '_0');
        event.stopPropagation();
        callback(button);
    });
    ...
    button.setHighlight = (value: boolean) => {
        button.data.set('highlighted', value);
...
const registerButtons = (scene: Phaser.Scene, config: any) => {
