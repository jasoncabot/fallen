...
export class MenuButton extends Phaser.GameObjects.Graphics {
    backgroundBounds: Phaser.Geom.Rectangle;
    text: Phaser.GameObjects.Text;

    constructor(scene: Phaser.Scene, rect: { x: number, y: number, width: number, height: number }, text: string, callback: (plugin: Phaser.Scenes.ScenePlugin) => void) {
        super(scene, { x: rect.x, y: rect.y });

        this.backgroundBounds = new Phaser.Geom.Rectangle(1, 1, rect.width - 1, rect.height - 1);

        this.text = scene.add.text(rect.x, rect.y + 8, text, { font: "16px Verdana", color: style.text.rest });
